...
if not LUNAR_AVAILABLE:
    st.error("🚨 [비상] 깃허브에 `requirements.txt` 파일이 없거나 설치 중입니다. 잠시만 기다려주세요.")
...
def calculate_user_saju(solar_date, time_str="모름"):
    ...
    if time_str != "모름":
        ...
        elif 19 <= h < 21: t_ji = 10 # 술시 (여기 오타 있었음)
        else: t_ji = 11 # 해시
...
def calculate_year_ganji_and_fortune(target_year, user_gan_idx, user_ji_idx):
    ...
    if abs(user_ji_idx - t_ji_idx) == 6: relation = f"⚠️ {y_name}의 지지와 본인의 일지가 충(沖)하여 변화와 변동이 많은 해입니다. 이사, 이직, 이동수가 강하니 신중한 판단이 필요합니다."
...
def get_monthly_flow_real(target_year_gan_idx, user_gan_idx):
    ...
    for i in range(12):
        ...
        elif rel_idx == 4: desc = "편재운: 뜻밖의 재물이나 사업적 성과가 따르며, 활동 범위가 넓어집니다."
        elif rel_idx == 5: desc = "정재운: 성실한 노력의 대가로 고정적인 수입이나 재물이 들어옵니다."
        elif rel_idx == 6: desc = "편관운: 과중한 업무나 책임감이 따르지만, 이를 극복하면 명예를 얻습니다."
        elif rel_idx == 7: desc = "정관운: 승진, 합격 등 명예운이 따르며, 조직 내에서 신용을 얻습니다."
        elif rel_idx == 8: desc = "편인운: 독특한 아이디어나 기술, 철학에 심취하기 좋으나 생각만 많아질 수 있습니다."
        elif rel_idx == 9: desc = "정인운: 윗사람의 도움이나 문서 계약, 학업 성취 등 인정을 받는 시기입니다."
