...
def calculate_user_saju(birth_date, time_str="모름"):
    ...
    # -------------------------------------------------------------
    # [1. 절기(Solar Terms) 계산] - 이게 없으면 다른 사람 사주가 틀립니다.
    # -------------------------------------------------------------
    # 각 달의 절기일 대략적 기준 (양력 4~8일 사이)
    # [1월(소한:6), 2월(입춘:4), 3월(경칩:6), 4월(청명:5), 5월(입하:6), 6월(망종:6)...]
    jeolgi_dates = [6, 4, 6, 5, 6, 6, 7, 8, 8, 8, 8, 7]
    ...
    # 해당 월의 절기일보다 생일이 빠르면 -> 전달로 침 (예: 6월 1일은 아직 5월 기운)
    ...
    # -------------------------------------------------------------
    # [4. 일주(Day) 계산] - 절대일수 계산법 (블루나잇님 기사일 기준)
    # -------------------------------------------------------------
    # 누구에게나 적용되는 수학적 계산입니다.
    # 기준점: 1960년 6월 26일 = 기사(己巳)일 (천간:5, 지지:5)
    ref_date = date(1960, 6, 26)
    ref_gan, ref_ji = 5, 5
    
    days_diff = (birth_date - ref_date).days
    
    gan_idx = (ref_gan + days_diff) % 10 # 일간
    ji_idx = (ref_ji + days_diff) % 12  # 일지
    ...
    if time_str != "모름":
        ...
        elif 5 <= hour < 7: time_ji_idx = 3 # 묘시
        elif 7 <= hour < 9: time_ji_idx = 4 # 진시
        elif 9 <= hour < 11: time_ji_idx = 5 # 사시
        elif 11 <= hour < 13: time_ji_idx = 6 # 오시
        elif 13 <= hour < 15: time_ji_idx = 7 # 미시
        elif 15 <= hour < 17: time_ji_idx = 8 # 신시
        elif 17 <= hour < 19: time_ji_idx = 9 # 유시
        elif 19 <= hour < 21: time_ji_idx = 10 # 술시
        else: time_ji_idx = 11 # 해시
        
        # 시두법(Time Stem Rule): 일간(gan_idx)에 따라 시간의 천간이 결정됨
        # 공식: (일간 * 2 + 시지) % 10
        start_time_gan = (gan_idx % 5 * 2) % 10
        ...
        time_luck = [
            ...
            "아침 토끼(卯)의 시간: 인정이 많고 다정다감하며, 자손 복이 있고 평온한 삶을 누립니다.",
            "오전 용(辰)의 시간: 포부가 크고 이상이 높으며, 사회적으로 큰 성취를 이룰 그릇입니다.",
            "오전 뱀(巳)의 시간: 두뇌 회전이 빠르고 깔끔하며, 전문 분야에서 이름을 떨칩니다.",
            "한낮 말(午)의 시간: 열정적이고 화려하며, 사교성이 좋아 인기가 많고 활동적입니다.",
            "오후 양(未)의 시간: 온화하고 침착하며, 예술적 감각이나 기술로 말년에 안정을 찾습니다.",
            "오후 원숭이(申)의 시간: 재주가 많고 임기응변이 뛰어나며, 다재다능하여 굶을 일이 없습니다.",
            "저녁 닭(酉)의 시간: 예민하지만 정확하고 꼼꼼하여, 재물을 잘 관리하고 빈틈이 없습니다.",
    ...
    personalities = [
        ...
        "타오르는 태양(丙)처럼 열정적이고 예의가 바르며 솔직합니다. 언제나 중심에 서기를 좋아합니다.", 
        "은은한 촛불(丁)처럼 섬세하고 따뜻하며 배려심이 깊습니다. 예술적 재능이 뛰어납니다.", 
        "거대한 산(戊)처럼 묵직하고 신용을 중시하며 포용력이 좋습니다. 중재자 역할을 잘합니다.", 
        "비옥한 텃밭(己)처럼 실속 있고 다재다능하며 교육적입니다. 실리를 잘 챙기는 실속파입니다.", 
        "단단한 바위(庚)처럼 의리가 강하고 결단력이 빠릅니다. 혁명가적 기질이 있습니다.", 
        "반짝이는 보석(辛)처럼 예민하고 깔끔하며 감수성이 풍부합니다. 전문가로 성공합니다.", 
        "도도한 강물(壬)처럼 지혜롭고 유연하며 임기응변에 강합니다. 리더나 지략가 스타일입니다.", 
        "스며드는 빗물(癸)처럼 치밀하고 조용히 내실을 다지는 지략가입니다. 기획력이 뛰어납니다."
    ]
    
    sinsal = ""
    if ji_idx in [0, 6, 3, 9]: sinsal = "✨ 도화살(桃花殺): 사람을 끄는 매력과 인기가 있습니다."
    elif ji_idx in [2, 8, 5, 11]: sinsal = "🐎 역마살(驛馬殺): 활동적이고 이동수가 많습니다."
    else: sinsal = "🎨 화개살(華蓋殺): 예술적 감각과 재주가 많습니다."

    month = birth_date.month
    season_advice = ""
    if month in [3, 4, 5]: season_advice = "봄(春)생: 금(金, 흰색) 기운으로 가지치기를 해주면 결실이 좋습니다."
    elif month in [6, 7, 8]: season_advice = "여름(夏)생: 수(水, 검정) 기운으로 열기를 식혀야 재물운이 삽니다."
    elif month in [9, 10, 11]: season_advice = "가을(秋)생: 화(火, 붉은색) 기운으로 따뜻하게 해야 인복이 옵니다."
    else: season_advice = "겨울(冬)생: 화(火, 붉은색) 기운을 가까이해야 만물이 소생하고 발복합니다."

    health_job_db = [
        ("간, 신경성", "기획, 교육, 건축"), ("위장, 허리", "예술, 유튜브, 언변"), ("심혈관, 시력", "방송, 전자, 미디어"),
        ("심장, 시력", "정밀기술, 상담, IT"), ("위장, 피부", "부동산, 중개, 컨설팅"), ("소화기", "교육, 비서, 부동산"),
        ("호흡기, 뼈", "군인, 경찰, 금속"), ("기관지, 폐", "금융, 의료, 반도체"), ("신장, 혈액", "무역, 유통, 해운"), ("신장, 생식기", "요식업, 심리, 약학")
    ]
    special_advice_db = [
        "갑목(甲)의 기개로 굽히지 않는 소신을 지키되, 주변을 보는 여유를 가지세요.", "을목(乙)의 유연함이 무기입니다. 끈기로 결국 승리합니다.",
        "병화(丙)의 빛으로 세상을 비추세요. 당신의 열정이 희망입니다.", "정화(丁)의 따스함으로 사람을 얻으세요. 배려가 복이 됩니다.",
        "무토(戊)의 신용이 자산입니다. 산처럼 버티면 재물이 모입니다.", "기토(己)의 실속을 챙기세요. 자신을 가꾸는 것이 성공 비결입니다.",
        "경금(庚)의 결단력이 길을 엽니다. 망설이지 말고 나아가세요.", "신금(辛)의 예리함으로 가치를 증명하세요. 당신은 보석입니다.",
        "임수(壬)의 지혜를 믿으세요. 유연하게 대처하면 못 넘을 산이 없습니다.", "계수(癸)의 꾸준함이 답입니다. 치밀하게 미래를 설계하세요."
    ]
    lucky_colors = ["초록색", "초록색", "붉은색", "붉은색", "노란색", "노란색", "흰색", "흰색", "검정색", "검정색"]
...
if go:
    ...
    with st.spinner(f'{target_year}년의 천기와 귀하의 사주를 정밀 대조 분석 중입니다...'):
        ...
        # 1. 내 정보 분석 (시간 포함!) - 여기가 수정된 핵심 포인트입니다!
    ...
    with res_area:
        ...
        if is_relation:
            with st.expander(f"👥 4. {t_name}님과의 관계 분석 (나와 상대방)"):
                ...
                st.write("---")
                st.write(f"**상대방 시간 분석:** {t_data['time_info']}")
        ...
        # 6. 오늘의 운세 (개운법) - [매일 바뀌도록 업그레이드 됨]
        with st.expander("🍀 6. 오늘의 행운 (매일 바뀝니다!)"):
            # 오늘 날짜 가져오기
            today = date.today()
            
            # [매일 바뀌는 로직] 오늘 날짜 숫자 + 내 사주 점수(gan_idx)를 더해서 '오늘의 랜덤 씨앗'을 만듦
            daily_seed = today.year + today.month + today.day + my_data['gan_idx']
            
            # 1. 오늘의 색상 (8가지 중 로테이션)
            color_list = ["열정의 빨간색", "차분한 파란색", "밝은 노란색", "깨끗한 흰색", "시크한 검은색", "신비로운 보라색", "생기 있는 초록색", "부유한 황금색"]
            ...
            # 3. 오늘의 방향 (8방위)
            dir_list = ["동쪽", "서쪽", "남쪽", "북쪽", "동남쪽", "동북쪽", "서남쪽", "서북쪽"]
            today_dir = dir_list[daily_seed % 8]

            st.info(f"📅 **{today.month}월 {today.day}일**, {u_name}님을 위한 행운의 처방입니다.")
            st.write(f"🎨 행운의 색상: **{today_color}**")
            st.write(f"🔢 행운의 숫자: **{today_num}**")
