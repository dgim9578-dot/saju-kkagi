import streamlit as st
from datetime import date, datetime, timedelta
import time
import os

# ============================================================================
# [0] 라이브러리 설정
# requirements.txt 파일에 korean_lunar_calendar 가 있어야만 작동합니다.
# ============================================================================
try:
    from korean_lunar_calendar import KoreanLunarCalendar
    LUNAR_AVAILABLE = True
except ImportError:
    LUNAR_AVAILABLE = False

# ============================================================================
# [1] 페이지 설정
# ============================================================================
st.set_page_config(page_title="사주까기 PRO - 마스터 평생 운세", layout="wide")

st.markdown("""
    <style>
    .main { background-color: #0f172a; color: white; }
    .stButton>button { 
        width: 100%; border-radius: 50px; 
        background: linear-gradient(45deg, #1e3a8a, #3b82f6);
        color: white; font-size: 18px; font-weight: bold; height: 4em;
        margin-top: 20px; box-shadow: 0 4px 15px rgba(0,0,0,0.3);
        transition: all 0.3s ease;
    }
    .stButton>button:hover { transform: scale(1.02); }
    .result-card { background-color: #1e293b; padding: 25px; border-radius: 15px; border: 1px solid #334155; margin-bottom: 20px; }
    .consult-box { 
        background: linear-gradient(135deg, #1e293b, #0f172a); 
        padding: 40px; border-radius: 20px; border: 2px solid #facc15; 
        text-align: center; margin-top: 30px; margin-bottom: 30px;
    }
    .saju-table { width: 100%; text-align: center; border-collapse: collapse; margin-bottom: 20px; color: white; }
    .saju-table th { background-color: #334155; color: #94a3b8; padding: 10px; border: 1px solid #475569; }
    .saju-table td { background-color: #1e293b; color: #facc15; font-size: 1.5em; font-weight: bold; padding: 20px; border: 1px solid #475569; }
    input, select { background-color: #334155 !important; color: white !important; border: 1px solid #64748b !important; }
    .stInfo, .stSuccess, .stWarning { color: white !important; font-weight: bold; }
    .stProgress > div > div > div > div { background-image: linear-gradient(to right, #4ade80, #3b82f6); }
    </style>
    """, unsafe_allow_html=True)

st.title("🔮 사주까기 PRO - 마스터 평생 운세")

if not LUNAR_AVAILABLE:
    st.error("🚨 [비상] 깃허브에 `requirements.txt` 파일이 없습니다. (내용: korean_lunar_calendar)")

# ============================================================================
# [2] 사용자 입력
# ============================================================================
st.write("### 📅 분석 연도 및 내 정보 입력")

target_year = st.number_input("분석하고 싶은 연도", min_value=2025, max_value=2050, value=2026)
u_name = st.text_input("성명", value="블루나잇")

col1, col2 = st.columns([1, 1])
with col1: u_cal_type = st.radio("내 생일 구분", ["양력", "음력"], horizontal=True)
with col2: 
    u_is_yundal = False
    if u_cal_type == "음력": u_is_yundal = st.checkbox("윤달(Leap Month) 입니까?")

st.write("▼ 출생 생년월일 (숫자로 직접 입력)")
c1, c2, c3 = st.columns([1.2, 1, 1])
with c1: u_year = st.number_input("년도", 1900, 2050, 1960, format="%d")
with c2: u_month = st.selectbox("월", list(range(1, 13)), 5)
with c3: u_day = st.number_input("일", 1, 31, 26, format="%d")

u_time = st.selectbox("출생 시간", [f"{i}시" for i in range(24)] + ["모름"], index=21)

is_relation = st.checkbox("👥 상대방(궁합) 보기")
t_name, t_year, t_month, t_day, t_time = "상대방", 1960, 1, 1, "모름"
t_cal_type = "양력"
t_is_yundal = False

if is_relation:
    st.write("---")
    t_name = st.text_input("상대방 성명", "상대방")
    tc1, tc2 = st.columns([1, 1])
    with tc1: t_cal_type = st.radio("상대방 구분", ["양력", "음력"], horizontal=True, key='t')
    with tc2: 
        if t_cal_type == "음력": t_is_yundal = st.checkbox("상대방 윤달")
    
    tc3, tc4, tc5 = st.columns([1.2, 1, 1])
    with tc3: t_year = st.number_input("상대방 년", 1900, 2050, 1960)
    with tc4: t_month = st.selectbox("상대방 월", list(range(1, 13)), 0)
    with tc5: t_day = st.number_input("상대방 일", 1, 31, 1)
    t_time = st.selectbox("상대방 시간", [f"{i}시" for i in range(24)] + ["모름"])

st.write("---")
go = st.button(f"{target_year}년 정밀 분석 리포트 생성 🔮")


# ============================================================================
# [엔진 0] 음력 -> 양력 변환기
# ============================================================================
def get_solar_date(year, month, day, is_lunar, is_leap):
    if not is_lunar:
        try: return date(year, month, day)
        except: return None
    
    if LUNAR_AVAILABLE:
        calendar = KoreanLunarCalendar()
        try:
            calendar.setLunarDate(year, month, day, is_leap)
            return date(calendar.solarYear, calendar.solarMonth, calendar.solarDay)
        except: return None
    else: return date(year, month, day)

# ============================================================================
# [엔진 1] 정통 만세력 엔진 (절기 포함, 함수명 통일됨)
# ============================================================================
def calculate_user_saju(solar_date, time_str="모름"):
    if solar_date is None: return None

    gan = ["갑(甲)", "을(乙)", "병(丙)", "정(丁)", "무(戊)", "기(己)", "경(庚)", "신(辛)", "임(壬)", "계(癸)"]
    ji = ["자(子)", "축(丑)", "인(寅)", "묘(卯)", "진(辰)", "사(巳)", "오(午)", "미(未)", "신(申)", "유(酉)", "술(戌)", "해(亥)"]
    
    # 24절기 (소한~대설)
    jeolgi_dates = [6, 4, 6, 5, 6, 6, 7, 8, 8, 8, 8, 7]
    
    year = solar_date.year
    month = solar_date.month
    day = solar_date.day

    # 1. 년주
    saju_year = year
    if month == 1 or (month == 2 and day < jeolgi_dates[1]): 
        saju_year = year - 1
    
    y_gan = (saju_year - 1900 + 6) % 10
    y_ji = (saju_year - 1900 + 0) % 12
    year_pillar = f"{gan[y_gan]}{ji[y_ji]}년"

    # 2. 월주 (절기 기준)
    m_base = (month + 10) % 12
    if day < jeolgi_dates[month - 1]:
        m_base = (m_base - 1 + 12) % 12
    
    m_ji = (m_base + 2) % 12
    start_m_gan = (y_gan % 5 * 2 + 2) % 10
    m_gan_val = (start_m_gan + m_base) % 10
    month_pillar = f"{gan[m_gan_val]}{ji[m_ji]}월"

    # 3. 일주 (절대일수)
    base_date = date(1900, 1, 1)
    delta = (solar_date - base_date).days
    d_gan = (0 + delta) % 10
    d_ji = (10 + delta) % 12
    day_pillar = f"{gan[d_gan]}{ji[d_ji]}일"

    # 4. 시주
    t_ji = 0
    my_time_ganji = "시간 모름"
    
    if time_str != "모름":
        h = int(time_str.replace("시", ""))
        if 23 <= h or h < 1: t_ji = 0
        elif 1 <= h < 3: t_ji = 1
        elif 3 <= h < 5: t_ji = 2
        elif 5 <= h < 7: t_ji = 3
        elif 7 <= h < 9: t_ji = 4
        elif 9 <= h < 11: t_ji = 5
        elif 11 <= h < 13: t_ji = 6
        elif 13 <= h < 15: t_ji = 7
        elif 15 <= h < 17: t_ji = 8
        elif 17 <= h < 19: t_ji = 9
        elif 19 <= hour < 21: t_ji = 10
        else: t_ji = 11
        
        t_gan = ((d_gan % 5 * 2) + t_ji) % 10
        my_time_ganji = f"{gan[t_gan]}{ji[t_ji]}시"
        
        msgs = [
            "늦은 밤 쥐(子)의 시간: 지혜롭고 신중하며, 말년에 재물을 숨겨두는 실속파입니다.",
            "새벽 소(丑)의 시간: 근면 성실하며, 뚝심 있게 자수성가하여 말년이 편안합니다.",
            "새벽 호랑이(寅)의 시간: 추진력이 강하고 활동적이며, 대기만성형으로 명예를 얻습니다.",
            "아침 토끼(卯)의 시간: 인정이 많고 다정다감하며, 자손 복이 있고 평온한 삶을 누립니다.",
            "오전 용(辰)의 시간: 포부가 크고 이상이 높으며, 사회적으로 큰 성취를 이룰 그릇입니다.",
            "오전 뱀(巳)의 시간: 두뇌 회전이 빠르고 깔끔하며, 전문 분야에서 이름을 떨칩니다.",
            "한낮 말(午)의 시간: 열정적이고 화려하며, 사교성이 좋아 인기가 많고 활동적입니다.",
            "오후 양(未)의 시간: 온화하고 침착하며, 예술적 감각이나 기술로 말년에 안정을 찾습니다.",
            "오후 원숭이(申)의 시간: 재주가 많고 임기응변이 뛰어나며, 다재다능하여 굶을 일이 없습니다.",
            "저녁 닭(酉)의 시간: 예민하지만 정확하고 꼼꼼하여, 재물을 잘 관리하고 빈틈이 없습니다.",
            "밤 개(戌)의 시간: 충직하고 신의가 있으며, 가정을 잘 지키고 내실을 다집니다.",
            "밤 돼지(亥)의 시간: 마음이 넓고 여유가 있으며, 식복이 타고나 말년이 풍요롭습니다."
        ]
        t_desc = f"당신은 **[{my_time_ganji}]**에 태어나셨군요. \n💡 {msgs[t_ji]}"
    else:
        t_desc = "태어난 시간을 알면 말년운과 자식운을 정확히 볼 수 있습니다."

    # [자료 100% 복구] 성격, 건강, 직업, 조언 등
    personalities = [
        "곧게 뻗은 소나무(甲)처럼 리더십이 강하고 자존심이 셉니다. 한번 목표를 정하면 뚫고 나가는 추진력이 대단하며, 굽히기 싫어하는 기질이 있어 직장보다는 사업이나 전문직이 어울립니다. 대쪽 같은 성품으로 타인의 모범이 됩니다.", 
        "유연한 화초(乙)처럼 적응력이 뛰어나고 끈기가 강인합니다. 겉은 부드러워 보이나 속은 아주 강인한 외유내강형으로, 환경을 활용하는 지혜가 뛰어나며 대인관계가 원만하여 인복이 있습니다.", 
        "타오르는 태양(丙)처럼 열정적이고 예의가 바르며 솔직합니다. 비밀을 담아두지 못하고 매사 화끈한 성격이며, 공명정대하게 일을 처리하려 하는 리더형입니다. 언제나 중심에 서기를 좋아합니다.", 
        "은은한 촛불(丁)처럼 섬세하고 따뜻하며 배려심이 깊습니다. 남을 배려하는 마음이 깊고 감수성이 풍부하나, 내면의 외로움이나 폭발적인 열정도 함께 지니고 있어 예술적 재능이 뛰어납니다.", 
        "거대한 산(戊)처럼 묵직하고 신용을 중시하며 포용력이 좋습니다. 믿음직스럽지만 고집이 세서 융통성이 필요할 때가 있으며, 모든 것을 품어주는 중재자 역할을 아주 잘 수행합니다.", 
        "비옥한 텃밭(己)처럼 실속 있고 다재다능하며 교육적입니다. 남을 기르고 가르치는 일에 탁월한 재능이 있으며, 현실적인 감각이 뛰어나 실리를 잘 챙기는 실속파입니다.", 
        "단단한 바위(庚)처럼 의리가 강하고 결단력이 빠릅니다. 맺고 끊음이 확실하여 혁명가적 기질을 가지고 있으며, 의협심이 강해 불의를 보면 참지 못하는 강인함이 있습니다.", 
        "반짝이는 보석(辛)처럼 예민하고 깔끔하며 감수성이 풍부합니다. 정확한 것을 좋아하며 미적 감각이 뛰어납니다. 섬세한 작업이나 비평, 분석에 남다른 소질이 있어 전문가로 성공합니다.", 
        "도도한 강물(壬)처럼 지혜롭고 유연하며 임기응변에 강합니다. 생각의 폭이 넓고 포용력이 있어 리더에 적합하나, 속을 알 수 없는 깊음과 유연함을 동시에 가집니다. 지략가 스타일입니다.", 
        "스며드는 빗물(癸)처럼 치밀하고 조용히 내실을 다지는 지략가입니다. 기획력과 아이디어가 매우 뛰어납니다. 작은 물방울이 바위를 뚫듯 꾸준함이 가장 큰 무기이며, 교육이나 활인업에 적합합니다."
    ]
    
    # [에러 수정] 변수명 colors 정의 (NameError 해결)
    colors = ["푸른색/초록색", "푸른색/초록색", "붉은색/분홍색", "붉은색/분홍색", "노란색/갈색", "노란색/갈색", "흰색/금색", "흰색/금색", "검은색/회색", "검은색/회색"]
    
    sinsal = "평범한 기운"
    if d_ji in [0, 6, 3, 9]: sinsal = "✨ 도화살(桃花殺): 사람을 끄는 매력과 인기가 있으며, 대인관계에서 주목받는 기운이 있습니다."
    elif d_ji in [2, 8, 5, 11]: sinsal = "🐎 역마살(驛馬殺): 활동적이고 이동수가 많으며, 타향이나 해외에서 성공할 운이 강합니다."
    else: sinsal = "🎨 화개살(華蓋殺): 예술적 감각과 종교적 철학성이 뛰어나며, 명예를 중시하고 재주가 많습니다."

    health_job_db = [
        ("간, 담, 신경성 두통, 근육통, 안구 피로", "기획, 교육, 목재, 출판, 언론, 건축, 가구, 인테리어"), 
        ("위장병, 허리 통증, 목 디스크, 신경 예민", "예술, 디자인, 유튜브, 프리랜서, 장식, 의류, 화훼"), 
        ("심혈관, 시력 저하, 고혈압, 소장", "에너지, 방송, 전자, 개인사업, 화려한 직업, 조명, 미디어"), 
        ("심장, 소장, 안구 건조, 가슴 두근거림", "정밀 기술, 상담, IT, 공동투자, 분석가, 연구직, 간호"), 
        ("위장, 복부 비만, 피부 트러블, 당뇨", "부동산, 중개, 농업, 컨설팅, 토목, 창고업, 종교"), 
        ("비장, 위장, 소화기 계통, 식습관 관리", "교육, 비서, 기록관리, 인허가, 원예, 요양, 부동산"), 
        ("호흡기, 폐, 뼈, 관절, 대장, 치아", "군인, 경찰, 금속 가공, 특수직, 운동선수, 경호, 기계"), 
        ("기관지, 폐, 불면증, 예민함, 면역력", "금융, 세무, 보석 세공, 공무원, 의료, 정밀기계, 반도체"), 
        ("신장, 방광, 혈액 순환, 비뇨기", "무역, 유통, 해운업, 주식, 외교, 숙박업, 물류"), 
        ("신장, 생식기, 냉증, 우울감, 수족냉증", "요식업, 서비스, 마트, 임대업, 심리상담, 약학, 식품") 
    ]
    
    special_advice_db = [
        "갑목(甲木)의 기개로 굽히지 않는 소신을 지키되, 주변을 둘러보는 여유를 가진다면 거목으로 성장할 것입니다.", 
        "을목(乙木)의 유연함이 최고의 무기입니다. 강한 비바람에도 꺾이지 않는 잡초 같은 끈기로 결국 승리할 것입니다.", 
        "병화(丙火)의 밝은 빛으로 세상을 비추세요. 당신의 솔직함과 열정은 주변 사람들에게 큰 희망이 됩니다.", 
        "정화(丁火)의 따스함으로 사람들의 마음을 녹이세요. 당신의 세심한 배려가 결국 큰 복으로 돌아옵니다.", 
        "무토(戊土)의 묵직함으로 신뢰를 쌓으세요. 급하게 서두르지 않아도 산처럼 버티면 사람과 재물이 모입니다.", 
        "기토(己土)의 포용력으로 내실을 다지세요. 화려함보다는 실속을 챙기며 자신을 가꾸는 것이 성공의 비결입니다.", 
        "경금(庚金)의 결단력으로 맺고 끊음을 확실히 하세요. 망설임 없는 당신의 의지가 새로운 길을 열어줄 것입니다.", 
        "신금(辛金)의 예리함으로 가치를 증명하세요. 보석은 갈고 닦을수록 빛나는 법이니 자신을 단련하는 데 힘쓰세요.", 
        "임수(壬水)의 지혜로 흐름을 타세요. 강물이 바다로 가듯, 유연하게 대처하면 어떤 장애물도 넘을 수 있습니다.", 
        "계수(癸水)의 치밀함으로 미래를 설계하세요. 작은 물방울이 모여 바위를 뚫듯, 꾸준함이 당신의 가장 큰 무기입니다." 
    ]
    
    return {
        "gan": gan[d_gan], "ji": ji[d_ji], "gan_idx": d_gan, "ji_idx": d_ji,
        "saju_full": {"year": year_pillar, "month": month_pillar, "day": day_pillar, "time": my_time_ganji},
        "desc": personalities[d_gan], "color": colors[d_gan],
        "hj_data": health_job_db[d_gan], "special_msg": special_advice_db[d_gan], 
        "sinsal": sinsal, "time_info": t_desc
    }

# ============================================================================
# [엔진 2] 평생 운세 자동 생성기
# ============================================================================
def calculate_year_ganji_and_fortune(target_year, user_gan_idx, user_ji_idx):
    t_gan_idx = (target_year - 4) % 10
    t_ji_idx = (target_year - 4) % 12
    gan = ["갑", "을", "병", "정", "무", "기", "경", "신", "임", "계"]
    ji = ["자", "축", "인", "묘", "진", "사", "오", "미", "신", "유", "술", "해"]
    y_name = f"{gan[t_gan_idx]}{ji[t_ji_idx]}년"
    
    diff = (t_gan_idx // 2 - user_gan_idx // 2 + 5) % 5
    titles = ["비겁운 (경쟁/주관)", "식상운 (활동/표현)", "재성운 (재물/결실)", "관성운 (명예/직장)", "인성운 (문서/공부)"]
    descs = [
        f"{target_year}년은 나와 같은 기운이 들어와 주관이 뚜렷해지고 독립심이 강해지는 시기입니다. 동료나 친구들과 협력하여 일을 도모하기 좋습니다. 다만, 지나친 고집은 경쟁과 마찰을 부를 수 있으니 주의하세요. 형제, 친구, 동료와의 관계가 중요해지는 시기입니다.",
        f"{target_year}년은 나의 기운을 밖으로 표출하는 시기입니다. 창의적인 아이디어가 샘솟고 활동력이 왕성해져 새로운 일을 기획하거나 창작 활동에 최적입니다. 의식주가 풍족해지고 능력을 인정받으나, 말실수를 조심해야 합니다.",
        f"{target_year}년은 노력의 대가인 재물이 따르는 해입니다. 사업 확장, 투자 수익 등 경제적인 성과가 기대되며, 현실적인 감각이 매우 발달하는 시기입니다.",
        f"{target_year}년은 책임감과 명예가 따르는 시기입니다. 취업, 승진, 합격 등 사회적 지위가 상승하지만, 과도한 업무로 인한 스트레스 관리가 필수입니다. 여성은 남성운이 들어오는 시기입니다.",
        f"{target_year}년은 윗사람의 덕을 보거나 문서 계약, 자격증 취득, 학업 성취에 매우 유리합니다. 내실을 다지며 미래를 준비하기 좋으며, 부동산 관련 운도 상승합니다."
    ]
    
    relation = "특별한 충돌 없이 무난하고 평온한 흐름이 예상됩니다."
    if abs(user_ji_idx - t_ji_idx) == 6: relation = f"⚠️ {y_name}의 지지와 본인의 일지가 충(沖)하여 변화가 많은 해입니다. 이사, 이직 등 이동수가 강합니다."
    elif (user_ji_idx + t_ji_idx) % 12 == 1: 
        relation = "❤️ 육합(六合)이 들어와 귀인의 도움을 받고 안정을 찾는 시기입니다. 주변과 화합하며 순조로운 한 해가 될 것입니다."
    elif (user_ji_idx == 0 and t_ji_idx == 6) or (user_ji_idx == 6 and t_ji_idx == 0): relation = "⚡ 자오충(子午沖): 물과 불의 충돌, 감정 변화와 건강 관리에 유의하세요."
    elif (user_ji_idx == 6 and t_ji_idx == 7) or (user_ji_idx == 7 and t_ji_idx == 6): relation = "❤️ 오미합(午未合): 화합과 안정이 깃드는 시기입니다."
    
    return y_name, titles[diff], descs[diff], relation, t_gan_idx

# [엔진 3] 진짜 만세력 월건 & 십성 (자료 복구)
def get_monthly_flow_real(target_year_gan_idx, user_gan_idx):
    start_gan_map = {0:2, 1:4, 2:6, 3:8, 4:0, 5:2, 6:4, 7:6, 8:8, 9:0}
    start_gan = start_gan_map[target_year_gan_idx]
    
    gan = ["갑", "을", "병", "정", "무", "기", "경", "신", "임", "계"]
    ji = ["인", "묘", "진", "사", "오", "미", "신", "유", "술", "해", "자", "축"]
    
    monthly_flow = []
    
    for i in range(12):
        m_gan_idx = (start_gan + i) % 10
        m_gan = gan[m_gan_idx]
        m_ji = ji[i]
        
        rel_idx = (m_gan_idx - user_gan_idx + 10) % 10
        
        desc = ""
        if rel_idx == 0: desc = "비견운: 나의 주관이 강해지고 친구나 동료와 협력하거나 경쟁하는 달입니다. 사람들 속에서 나의 세력을 키우세요."
        elif rel_idx == 1: desc = "겁재운: 경쟁심이 발동하며 재물 지출에 유의해야 하는 시기입니다. 동료와의 관계에서 실속을 챙겨야 합니다."
        elif rel_idx == 2: desc = "식신운: 의식주가 풍족하고 나의 재능을 자연스럽게 발휘하는 즐거운 시기입니다. 창의적인 활동이 유리합니다."
        elif rel_idx == 3: desc = "상관운: 관습을 타파하고 새로운 아이디어를 내는 시기입니다. 화려한 언변이 돋보이나 구설수를 조심하세요."
        elif rel_idx == 4: desc = "편재운: 뜻밖의 재물이나 사업적 성과가 따르며, 활동 범위가 넓어지고 바빠지는 역동적인 시기입니다."
        elif rel_idx == 5: desc = "정재운: 성실한 노력의 대가로 고정적인 수입이나 재물이 들어오며, 안정을 추구하는 실속 있는 시기입니다."
        elif rel_idx == 6: desc = "편관운: 과중한 업무나 책임감이 따르지만, 이를 극복하면 큰 명예와 권위를 얻게 되는 시기입니다."
        elif rel_idx == 7: desc = "정관운: 승진, 합격 등 명예운이 따르며, 조직 내에서 신용을 얻고 안정을 누리는 반듯한 시기입니다."
        elif rel_idx == 8: desc = "편인운: 독특한 아이디어나 기술, 철학에 심취하기 좋으나, 생각이 많아져 활동성은 다소 위축될 수 있습니다."
        elif rel_idx == 9: desc = "정인운: 윗사람의 도움이나 문서 계약, 학업 성취 등 인정을 받는 시기입니다. 마음이 편안해집니다."
        
        monthly_flow.append(f"{i+1}월({m_gan}{m_ji}월): {desc}")
        
    return monthly_flow

def calculate_oheng_graph(gan_idx):
    scores = {'목': 10, '화': 10, '토': 10, '금': 10, '수': 10}
    if gan_idx in [0, 1]: scores['목'] += 40
    elif gan_idx in [2, 3]: scores['화'] += 40
    elif gan_idx in [4, 5]: scores['토'] += 40
    elif gan_idx in [6, 7]: scores['금'] += 40
    elif gan_idx in [8, 9]: scores['수'] += 40
    return scores

def get_gimun(date_obj):
    val = (date_obj.year + date_obj.month + date_obj.day) % 9
    if val == 0: val = 9
    g_data = {
        1: ("일백수", "지혜/철학", "깊은 물처럼 속을 알 수 없는 지혜와 유연함을 가졌습니다."), 
        2: ("이흑토", "안정/기반", "대지처럼 만물을 품어주는 모성애와 끈기가 있습니다."), 
        3: ("삼벽목", "시작/추진", "천둥처럼 뚫고 나가는 강력한 기획력과 실행력이 있습니다."),
        4: ("사록목", "교류/신용", "바람처럼 어디든 통하는 사교성과 부드러운 카리스마가 있습니다."), 
        5: ("오황토", "중심/제왕", "황제처럼 중앙에서 전체를 통솔하는 강력한 리더십이 있습니다."), 
        6: ("육백금", "권위/결단", "하늘의 법처럼 공정하고 냉철한 판단력과 결단력이 있습니다."),
        7: ("칠적금", "언변/재치", "입을 통해 즐거움을 주고 설득하는 뛰어난 언변이 있습니다."), 
        8: ("팔백토", "개혁/뚝심", "높은 산처럼 묵묵히 버티며 결국 이루어내는 개혁가입니다."), 
        9: ("구자화", "명예/열정", "타오르는 불꽃처럼 화려함을 추구하고 명예를 중시합니다.")
    }
    return val, g_data[val]

# ============================================================================
# [메인 실행 로직]
# ============================================================================
if go:
    res_area = st.container()
    with st.spinner(f'{target_year}년의 천기와 귀하의 사주를 정밀 대조 분석 중입니다...'):
        time.sleep(1.0)
        
        # 1. 날짜 변환 (음력 -> 양력)
        final_birth = get_solar_date(u_year, u_month, u_day, (u_cal_type=="음력"), u_is_yundal)
        
        if final_birth is None:
            st.error("🚨 날짜 오류: 존재하지 않는 날짜이거나 변환에 실패했습니다.")
        else:
            # 2. 사주 분석 (함수명 calculate_user_saju 사용)
            data = calculate_user_saju(final_birth, u_time)
            s = data['saju_full']
            oheng_scores = calculate_oheng_graph(data['gan_idx'])
            g_num, (g_title, g_key, g_desc) = get_gimun(final_birth)
            
            # 3. 운세 분석
            t_year_name, t_fortune_title, t_fortune_desc, t_relation_luck, t_gan_idx = calculate_year_ganji_and_fortune(target_year, data['gan_idx'], data['ji_idx'])
            
            # 4. 월별 운세
            monthly_data = get_monthly_flow_real(t_gan_idx, data['gan_idx'])

            with res_area:
                st.subheader(f"✨ {u_name}님의 {target_year}년({t_year_name}) 정밀 리포트")
                
                if u_cal_type == "음력":
                    st.info(f"ℹ️ 입력하신 **음력 {u_year}.{u_month}.{u_day}**을 **양력 {final_birth.year}.{final_birth.month}.{final_birth.day}**로 변환하여 분석했습니다.")

                # 1. 사주 원국
                with st.expander("📅 1. 타고난 사주 원국 (만세력 정밀)", expanded=True):
                    st.markdown(f"<h1 style='text-align:center; color:#facc15;'>{data['gan']}{data['ji']}</h1>", unsafe_allow_html=True)
                    st.caption("▲ 본인의 일주 (타고난 기운)")
                    
                    st.markdown(f"""
                        <table class="saju-table">
                            <tr><th>시주 (Time)</th><th>일주 (Day)</th><th>월주 (Month)</th><th>년주 (Year)</th></tr>
                            <tr>
                                <td>{s['time']}</td>
                                <td style="color:#facc15; border: 2px solid #facc15;">{s['day']}</td>
                                <td>{s['month']}</td>
                                <td>{s['year']}</td>
                            </tr>
                            <tr><td>말년/자식</td><td>나 자신(본원)</td><td>사회/부모</td><td>조상/초년</td></tr>
                        </table>
                    """, unsafe_allow_html=True)

                    st.write(f"**타고난 성향:** {data['desc']}")
                    st.write(f"💡 {data['sinsal']}")
                    if u_time != "모름":
                        st.success(data['time_info'])

                # 2. 운세
                with st.expander(f"🔥 2. {target_year}년 총운"):
                    st.markdown(f"### {t_fortune_title}")
                    st.write(t_fortune_desc)
                    st.warning(f"⚠️ **올해의 조언:** {t_relation_luck}")

                # 3. 기문정명학
                with st.expander(f"🔮 3. [특급] 기문정명학 해단 (선천수 {g_num})"):
                    st.markdown(f"### **{g_title} - {g_key}**")
                    st.write(g_desc)
                    st.write("타고난 명국(命局)이 중궁의 힘을 받아 사방으로 뻗어나가는 기세입니다.")

                # 4. 상대방 분석 (복구됨)
                if is_relation:
                    t_final = get_solar_date(t_year, t_month, t_day, (t_cal_type=="음력"), t_is_yundal)
                    if t_final:
                        t_data = calculate_user_saju(t_final, t_time)
                        if t_data:
                            ts = t_data['saju_full']
                            with st.expander(f"👥 4. {t_name}님과의 관계 분석 (나와 상대방)"):
                                st.markdown("### ❤️ 궁합 및 관계 정밀 진단")
                                st.write(f"상대방({t_name})님은 **{ts['day']} ({t_data['gan']}{t_data['ji']}) 일주**입니다.")
                                st.write(f"**성향:** {t_data['desc']}")
                                if data['gan_idx'] == t_data['gan_idx']: st.success("친구 같은 사이 (비견)")
                                elif abs(data['gan_idx'] - t_data['gan_idx'])==5: st.success("강하게 끌리는 사이 (천간합)")
                                else: st.info("서로 맞춰가는 사이")

                # 5. 월별 흐름
                with st.expander(f"📆 5. {target_year}년 월별 운세 흐름"):
                    for m in monthly_data: st.write(f"- {m}")

                # 6. 오늘의 운세 (복구됨)
                with st.expander("🍀 6. 오늘의 행운 (매일 바뀝니다!)"):
                    today = date.today()
                    daily_seed = today.year + today.month + today.day + data['gan_idx']
                    
                    color_list = ["열정의 빨간색", "차분한 파란색", "밝은 노란색", "깨끗한 흰색", "시크한 검은색", "신비로운 보라색", "생기 있는 초록색", "부유한 황금색"]
                    today_color = color_list[daily_seed % 8]
                    today_num = (daily_seed % 9) + 1
                    dir_list = ["동쪽", "서쪽", "남쪽", "북쪽", "동남쪽", "동북쪽", "서남쪽", "서북쪽"]
                    today_dir = dir_list[daily_seed % 8]

                    st.info(f"📅 **{today.month}월 {today.day}일**, {u_name}님을 위한 행운의 처방입니다.")
                    st.write(f"🎨 행운의 색상: **{today_color}**")
                    st.write(f"🔢 행운의 숫자: **{today_num}**")
                    st.write(f"🧭 행운의 방향: **{today_dir}**")
                    st.caption(f"※ 이 결과는 매일 자정이 지나면 내 사주 흐름에 맞춰 새롭게 바뀝니다.")

                # 7. 재물/건강/직업 (복구됨)
                with st.expander("💰 7. 재물·건강·직업 정밀 분석"):
                    health_txt, job_txt = data['hj_data']
                    st.markdown("### 🏥 건강 관리 비법")
                    st.warning(f"**주의:** {health_txt} 쪽에 유의하며, 미리 건강검진을 챙기세요.")
                    st.write("---")
                    st.markdown("### 💼 직업 및 재물 운")
                    st.success(f"**추천 분야:** {job_txt} 분야에서 두각을 나타낼 수 있습니다.")

                # 8. 오행 그래프 (복구됨)
                with st.expander("🐯 8. 나만의 오행(五行) 에너지 분포도"):
                    st.write(f"**{u_name}**님의 사주에 내재된 오행 에너지의 비율입니다.")
                    st.write(f"🌲 목(나무): {oheng_scores['목']}%")
                    st.progress(min(oheng_scores['목'], 100))
                    st.write(f"🔥 화(불): {oheng_scores['화']}%")
                    st.progress(min(oheng_scores['화'], 100))
                    st.write(f"⛰️ 토(흙): {oheng_scores['토']}%")
                    st.progress(min(oheng_scores['토'], 100))
                    st.write(f"⚔️ 금(쇠): {oheng_scores['금']}%")
                    st.progress(min(oheng_scores['금'], 100))
                    st.write(f"💧 수(물): {oheng_scores['수']}%")
                    st.progress(min(oheng_scores['수'], 100))
                    st.caption("※ 그래프가 높은 기운은 장점이나, 너무 과하면 오히려 독이 될 수 있습니다.")

                # 9. 특별 조언 (복구됨)
                with st.expander("📝 9. 사주까기의 특별 조언"):
                    st.markdown(f"### 💌 {u_name}님을 위한 한마디")
                    st.success(f"{data['special_msg']}")

                # 상담 섹션 및 QR (복구됨)
                st.markdown("---")
                st.markdown("""
                    <div class='consult-box'>
                        <h2 style='color: #facc15;'>🔮 "내 사주에 딱 맞는 더 깊은 해답"이 필요하신가요?</h2>
                        <p style='font-size: 1.2em; color: white; margin-top: 10px;'>
                            그렇다면 지금 <b>010-8393-7471 상담예약</b> 또는 <b>QR을 스캔</b> 하세요
                        </p>
                    </div>
                """, unsafe_allow_html=True)

                _, col_qr, _ = st.columns([1.2, 1, 1.2]) 
                with col_qr:
                    qr_path = "my_QR.jpg"
                    desktop_path = os.path.join(os.path.expanduser("~"), "Desktop", "my_QR.jpg")    
                    
                    if os.path.exists(qr_path):
                        st.image(qr_path, width=300)
                        st.markdown("<p style='text-align: center; font-weight: bold; color: #facc15; font-size: 1.2em;'>사주까기 전문가 상담 QR</p>", unsafe_allow_html=True)
                    elif os.path.exists(desktop_path):
                        st.image(desktop_path, width=300)
                        st.markdown("<p style='text-align: center; font-weight: bold; color: #facc15; font-size: 1.2em;'>사주까기 전문가 상담 QR</p>", unsafe_allow_html=True)
                    else:
                        st.info("⚠️ 'my_QR.jpg' 사진을 이 폴더나 바탕화면에 넣어주세요.")

st.write("---")
st.info("💡 본 리포트는 사주까기 님의 음양오행 정밀 알고리즘을 바탕으로 생성되었습니다.")